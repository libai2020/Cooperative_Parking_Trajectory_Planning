function [x, y, theta, fitness] = SearchTrajectoryInXYTGraph(bv)
global xyt_graph_search_ environment_scale_
start_ind = ConvertConfigToIndex(bv.x0, bv.y0, 0);
goal_ind = ConvertConfigToIndex(bv.xtf, bv.ytf, xyt_graph_search_.max_t);
[ind_vec, theta, fitness] = SearchViaAStar(start_ind, goal_ind, bv.theta0);
...
function [ind_vec, theta, fitness] = SearchViaAStar(start_ind, goal_ind, theta0)
...
if (any(inpolygon(original_obstacle_layers{1,ind(3)}.x, original_obstacle_layers{1,ind(3)}.y, V.x, V.y)))
    is_valid = 0;
else
    is_valid = 1;
end
